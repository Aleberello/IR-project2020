...
class Scraper:
    ...
    def __init__(self, handle, multiUsr=False):
        ...
        if multiUsr:
            self.outfile = "out/all_usrs.json"
        else:
            self.outfile = "out/" + self.handle + ".json"
    ...
    def scrape(self, start, end, by, loading_delay):
        ...
        if True: #self.__can_quickscrape():
            pprint(g("user ") + w("@") + y(self.handle) + g(" has fewer than"), y(TWEET_LIMIT), g("tweets, conducting quickscrape..."))
...
def init_chromedriver(debug=False):
    ...
    return webdriver.Chrome("./utils/chromedriver", options=options)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-us", "--usernames", help="Scrape user's Tweets in txt file.")
    ...
    if args.username is None:
        if args.usernames is not None: # multi-user search
            try:
                users = pd.read_csv(args.usernames, header=None)
            except:
